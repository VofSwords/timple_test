...
const handleTouchMove = (event: TouchEvent) => {
  ...
  if (element) {
    const starIndex = rates.findIndex((rate) =>
      element.closest(`[data-rate="${rate}"]`)
    );
    if (starIndex !== -1) {
      currentRate.value = starIndex + 1;
    }
  }
};

watch([currentRate, isSelecting], () => {
  if (!isSelecting.value) {
    emit("rateChanged", currentRate.value);
  }
});

const emit = defineEmits<{
  (e: "rateChanged", value: number): void;
}>();
</script>
...
<template>
  <div>
    ...
    <div
      class="mt-3 h-[68px] bg-white rounded-[20px] flex items-center justify-center gap-4 select-none"
      @mouseup="isSelecting = false"
      @mouseleave="isSelecting = false"
      @touchstart.stop.prevent="isSelecting = true"
      @touchcancel="isSelecting = false"
      @touchend="isSelecting = false"
    ...
    >
      ...
      >
        <svg
          ...
          :opacity="!isSelecting || currentRate < i ? 1 : 0.7"
        >
          <path
            ...
            :fill="currentRate < i ? '#CBD1D7' : '#9771FD'"
          />
        </svg>
      </div>
    </div>
